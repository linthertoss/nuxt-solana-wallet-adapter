...
export const useWallet = (): WalletStore => {
  ...
  throw new Error(
    "Wallet not initialized. Please use the `WalletProvider` component to initialize the wallet."
  );
};
...
export const createWalletStore = ({
  ...
  const name: Ref<WalletName | null> =
    useLocalStorage<WalletName>(localStorageKey);
  ...
  const walletsByName = computed(() => {
    return wallets.reduce<Record<any, Wallet>>((walletsByName, wallet) => {
  ...
  watch(
    name,
    (): void => {
      const wallet = walletsByName.value?.[name.value as any] ?? null;
